...
data_no_outliers = [(x,y) for x,y in zip(X,Y) if mean-2*stdev < y < mean+2*stdev]
print(data_no_outliers)
X_no_outliers = [x[0] for x in data_no_outliers]
Y_no_outliers = [x[1] for x in data_no_outliers]
sma_length = floor(len(Y_no_outliers)/2)
Y_sma_week_no_outliers = sma(Y_no_outliers,sma_length)
...
plt.plot(X_no_outliers[sma_length:],Y_sma_week_no_outliers,label='SMA ' + str(sma_length) + ' sales (no outliers)')
